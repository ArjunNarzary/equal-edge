...
export const CountryGroupTable = pgTable("country_groups", {
  id: uuid("id").primaryKey().defaultRandom(),
  name: text("name").notNull().unique(),
  recommendedDiscountPercentage: real("recommended_descount_percentage"),
  createdAt,
  updatedAt,
})
...
export const CountryGroupDiscountTable = pgTable(
  "country_group_discounts",
  {
    ...
    discountPercentage: real("discount_peracantage").notNull(),
    createdAt,
    updatedAt,
  },
  (table) => ({
...
export const countryGroupDiscountRealtions = relations(
